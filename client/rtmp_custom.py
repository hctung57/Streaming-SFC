...
class RTMP(object):
    ...
    def read_packet(self):
        ...
        send_request_time = monotonic()
        start_receive_packet = 0
        start_receive = False
        # print("send request time:",send_request_time)
        while not packet_complete:
            ...
            if start_receive == False:
                start_receive_packet = monotonic()
                start_receive = True
            packet_complete = packet.m_nBytesRead == packet.m_nBodySize
        receive_packet_complete = monotonic()
        return RTMPPacket._from_pointer(packet), send_request_time, start_receive_packet, receive_packet_complete
