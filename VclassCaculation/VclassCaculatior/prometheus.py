...
def Monitoring_Node(node_name):
    ...
    CPU_Utilisation = prom.custom_query(
        query=f"100 - sum(irate(node_cpu_seconds_total{job}[1m]) *100)")
    CPU_Usage = CPU_Utilisation[0].get('value')
    time = datetime.fromtimestamp(
        CPU_Usage[0]).strftime("%A, %B %d, %Y %I:%M:%S")
    CPU_Usage_value = CPU_Usage[1]

    Memory_Utilisation = prom.custom_query(
        query=f" ((node_memory_MemTotal_bytes{job} - node_memory_MemFree_bytes{job} - node_memory_Cached_bytes{job} - node_memory_Buffers_bytes{job}) / node_memory_MemTotal_bytes{job}) * 100 ")
    Memory_Usage = Memory_Utilisation[0].get('value')
    Memory_Usage_value = Memory_Usage[1]

    Bandwidth = prom.custom_query(
        query=f"sum(rate(node_network_receive_bytes_total{job}[1m]) + rate(node_network_transmit_bytes_total{job}[1m])) ")
...
def write_cvs_node(node_name):
    ...
    mydict = [{'Node_name': node_name, 'CPU(%)': CPU_Usage_value, 'Memory(%)': Memory_Usage_value,
               'Bandwidth': Bandwidth_value, 'Time': time_node}]
...
def write_cvs_pods(pod_name):
    ...
    mydict = [{'Pod_name': pod_name, 'CPU(%)': CPU_Pod_value, 'Memory(%)': Memory_Pod_value,
               'Bandwidth': Bandwidth_Pod_value, 'Time': time_pods}]
...
try:
    ...
    CPU_Usage_value, Memory_Usage_value, Bandwidth_value, time_node = Monitoring_Node(
        'node_worker_1')
    write_cvs_node('node_worker_1')
    for i in api_response.items:
        pod_name = i.metadata.name
        CPU_Pod_value, Memory_Pod_value, Bandwidth_Pod_value, time_pods = Monitoring_Pod(
            pod_name)
